...
Int64U Tmp;
  ...
  if(MmcDskCtrlBlk.BlockSize != 512)
  {
    // because Window support only 512 bytes block
    MmcDskCtrlBlk.BlockNumb *= (MmcDskCtrlBlk.BlockSize>>9);
    MmcDskCtrlBlk.BlockSize = 512;
  }
  // Set Write Protect
...
void MmcStatusUpdate (void)
{
  ...
  MmcDskCtrlBlk.WriteProtect = MmcWriteProtect() |\
                              bMmcPermWriteProtect;
  if((MmcDskCtrlBlk.DiskStatus != DiskCommandPass) | !MmcPresent())
  {
    switch (MmcInitMedia())
    {
    case MmcOk:
