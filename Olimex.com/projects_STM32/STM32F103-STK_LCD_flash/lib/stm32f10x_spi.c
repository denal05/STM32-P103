...
void I2S_Init(SPI_TypeDef* SPIx, I2S_InitTypeDef* I2S_InitStruct)
{
  ...
  assert(IS_SPI_23_PERIPH(SPIx));
  assert(IS_I2S_MODE(I2S_InitStruct->I2S_Mode));
  assert(IS_I2S_STANDARD(I2S_InitStruct->I2S_Standard));
  assert(IS_I2S_DATA_FORMAT(I2S_InitStruct->I2S_DataFormat));
  assert(IS_I2S_MCLK_OUTPUT(I2S_InitStruct->I2S_MCLKOutput));
  assert(IS_I2S_AUDIO_FREQ(I2S_InitStruct->I2S_AudioFreq));
  assert(IS_I2S_CPOL(I2S_InitStruct->I2S_CPOL));
...
void SPI_I2S_ITConfig(SPI_TypeDef* SPIx, u8 SPI_I2S_IT, FunctionalState NewState)
{
  ...
  assert(IS_SPI_ALL_PERIPH(SPIx));
  assert(IS_FUNCTIONAL_STATE(NewState));
  assert(IS_SPI_I2S_CONFIG_IT(SPI_I2S_IT));
...
void SPI_I2S_DMACmd(SPI_TypeDef* SPIx, u16 SPI_I2S_DMAReq, FunctionalState NewState)
{
  ...
  assert(IS_SPI_ALL_PERIPH(SPIx));
  assert(IS_FUNCTIONAL_STATE(NewState));
  assert(IS_SPI_I2S_DMAREQ(SPI_I2S_DMAReq));
...
FlagStatus SPI_I2S_GetFlagStatus(SPI_TypeDef* SPIx, u16 SPI_I2S_FLAG)
{
  ...
  /* Check the parameters */
  assert(IS_SPI_ALL_PERIPH(SPIx));
  assert(IS_SPI_I2S_GET_FLAG(SPI_I2S_FLAG));
...
void SPI_I2S_ClearFlag(SPI_TypeDef* SPIx, u16 SPI_I2S_FLAG)
{
  /* Check the parameters */
  assert(IS_SPI_ALL_PERIPH(SPIx));
  assert(IS_SPI_I2S_CLEAR_FLAG(SPI_I2S_FLAG));
...
ITStatus SPI_I2S_GetITStatus(SPI_TypeDef* SPIx, u8 SPI_I2S_IT)
{
  ...
  /* Check the parameters */
  assert(IS_SPI_ALL_PERIPH(SPIx));
  assert(IS_SPI_I2S_GET_IT(SPI_I2S_IT));
...
void SPI_I2S_ClearITPendingBit(SPI_TypeDef* SPIx, u8 SPI_I2S_IT)
{
  ...
  /* Check the parameters */
  assert(IS_SPI_ALL_PERIPH(SPIx));
  assert(IS_SPI_I2S_CLEAR_IT(SPI_I2S_IT));
